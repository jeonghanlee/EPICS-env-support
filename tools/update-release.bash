...
function _process_release_file
{
    ...
    # Array to store modules that need date comment updates
    # Format: "MODULE_SUFFIX|YYYY-MM-DD"
                    ...
                    while true; do
                        ...
                        case "$choice" in
                            ...
                            2)
                                ...
                                # Store module and date for post-processing comment update
                                DATE_UPDATES+=("${current_module_suffix}|${head_date}")
                                break
                                ;;
                            3)
    ...
    # --- Post-Processing: Update Date Comments ---
    # This block iterates through modules updated via Option 2 and updates
    # the comment line immediately preceding SRC_NAME_MODULE:=
    if [ ${#DATE_UPDATES[@]} -gt 0 ]; then
        printf "%bUpdating revision dates in comments...%b\n" "${CYAN}" "${NC}"
        for entry in "${DATE_UPDATES[@]}"; do
            local mod="${entry%%|*}"
            local new_date="${entry##*|}"
            
            # Use sed to find SRC_NAME_MOD:= and replace the preceding comment line.
            # $!N; ... P; D pattern creates a 2-line window to modify the line BEFORE the match.
            sed -i -e '$!N; s|^#.*\nSRC_NAME_'"${mod}"':=|# '"${new_date}"'\nSRC_NAME_'"${mod}"':=|; P; D' "$NEW_FILE"
        done
    fi
    # ---------------------------------------------
}
